...
echo CHTML::dibujaEtiqueta('br');
echo CHTML::campoText('nombre', $filtrado['nombre'], ['class' => 'form-control-sm', 'style' => 'height: 30px;']);
...
$tabla = new CGrid($cabecera, $tarifas, ['class' => 'table table-striped table-bordered d-none d-md-table']); 
$tablaHtml = $tabla->dibujate();

echo CHTML::dibujaEtiqueta('div', ['class' => 'p-5 position-relative']); 
echo $tablaHtml; // Tabla de tarifas
echo CHTML::dibujaEtiqueta('h1', ['class' => 'position-absolute top-0 translate-middle start-50 text-center textoFondo', 'style' => 'z-index: -1;'], 'TARIFAS'); 
...
foreach ($tarifas as $tarifa =>  $prop) {
    echo CHTML::dibujaEtiqueta('div', ['class' => 'col-12 mb-3'], '', false);
    echo CHTML::dibujaEtiqueta('div', ['class' => 'card'], '', false);
...
echo CHTML::link(
    'Nueva tarifa',
    Sistema::app()->generaURL(['tarifas', 'nuevo']),
